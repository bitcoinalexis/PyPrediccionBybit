...
class Visualizador:
    ...
    def crear_grafico_precios(self, fig, df, prediccion_futura=None, 
                             order_book=None, titulo=None):
        """
        ...
        for i in range(len(df)):
            ...
            ax_precios.plot([df.iloc[i]['datetime'], df.iloc[i]['datetime']], 
        ...
        if 'ma7' in df.columns:
            ax_precios.plot(df['datetime'], df['ma7'], color=COLORES['verde'], 
                           linewidth=1, alpha=0.8, label='MA7')
        if 'ma21' in df.columns:
            ax_precios.plot(df['datetime'], df['ma21'], color=COLORES['rojo'], 
                           linewidth=1, alpha=0.8, label='MA21')
        if 'ma50' in df.columns:
            ax_precios.plot(df['datetime'], df['ma50'], color=COLORES['azul'], 
                           linewidth=1, alpha=0.6, label='MA50')
            
        # Añadir bandas de Bollinger si existen
        if all(col in df.columns for col in ['bb_upper', 'bb_lower']):
            ax_precios.plot(df['datetime'], df['bb_upper'], color=COLORES['gris'], 
                           linestyle='--', linewidth=1, alpha=0.6)
            ax_precios.plot(df['datetime'], df['bb_lower'], color=COLORES['gris'], 
                           linestyle='--', linewidth=1, alpha=0.6)
        ...
        if prediccion_futura is not None:
            ...
            if fechas_futuras and precios_futuros:
                ...
                ax_precios.plot(fechas_futuras, precios_futuros, color=color_prediccion, 
                               linestyle='--', linewidth=2, label='Predicción')
                
                # Añadir área sombreada para mostrar incertidumbre
                std_estimado = df['close'].std() * 0.1  # 10% de desviación estándar histórica
                precios_sup = [p + std_estimado * (i+1) for i, p in enumerate(precios_futuros)]
                precios_inf = [p - std_estimado * (i+1) for i, p in enumerate(precios_futuros)]
                
                ax_precios.fill_between(fechas_futuras, precios_inf, precios_sup, 
                                      color=color_prediccion, alpha=0.2)
                ...
                if direccion > 0:  # Tendencia alcista
                    ax_precios.scatter(ultima_fecha, ultimo_precio * 1.005, 
                                     marker='^', s=80, color=COLORES['verde'])
                elif direccion < 0:  # Tendencia bajista
                    ax_precios.scatter(ultima_fecha, ultimo_precio * 0.995, 
                                     marker='v', s=80, color=COLORES['rojo'])
                
                # Conectar último precio conocido con predicción
                ax_precios.plot([ultima_fecha, fechas_futuras[0]], 
                              [ultimo_precio, precios_futuros[0]], 
                              color=color_prediccion, alpha=0.5, linewidth=1)
        ...
        if 'target_long' in df.columns and 'target_short' in df.columns:
            ...
            if len(indices_compra) > 0:
                ...
                precios_compra = df.loc[indices_compra, 'low'] * 0.998  # Ligeramente debajo
                ax_precios.scatter(fechas_compra, precios_compra, 
                                 marker='^', s=60, color=COLORES['verde'], alpha=0.7)
            ...
            if len(indices_venta) > 0:
                ...
                precios_venta = df.loc[indices_venta, 'high'] * 1.002  # Ligeramente arriba
                ax_precios.scatter(fechas_venta, precios_venta, 
                                 marker='v', s=60, color=COLORES['rojo'], alpha=0.7)
        ...
        if order_book and 'best_bid' in order_book and 'best_ask' in order_book:
            ...
            # Añadir etiquetas
            ax_precios.text(ultima_fecha, bid * 0.9998, f"Bid: {bid:.2f}", 
                          color=COLORES['verde'], ha='right', fontsize=8)
            ax_precios.text(ultima_fecha, ask * 1.0002, f"Ask: {ask:.2f}", 
                          color=COLORES['rojo'], ha='right', fontsize=8)
        ...
        if 'volume' in df.columns:
            for i in range(len(df)):
                ...
                ax_volumen.bar(df.iloc[i]['datetime'], df.iloc[i]['volume'], 
    ...
    def crear_mapa_calor(self, fig, corr_matrix):
        ...
        sns.heatmap(corr_matrix, mask=mask, annot=True, fmt=".2f", 
                   linewidths=0.5, cmap=cmap, center=0, ax=ax, 
                   annot_kws={"size": 8})
    ...
    def crear_mapa_calor_dinamico(self, fig, heatmap_df, ultimas_filas=20):
        ...
        # Crear el heatmap
        sns.heatmap(heatmap_df.T, cmap=cmap, center=0, 
    ...
    def crear_panel_indicadores(self, fig, resultado_prediccion):
        ...
        # Añadir etiquetas
        ax_signals.text(-0.7, 0.2, "SHORT", color=COLORES['rojo'], fontsize=10, ha='center')
        ax_signals.text(0.7, 0.2, "LONG", color=COLORES['verde'], fontsize=10, ha='center')
        ax_signals.text(0, -0.3, decision, color=color_decision, fontsize=14, weight='bold', ha='center')
        ...
        if indicators:
            ...
            for i, bar in enumerate(bars):
                width = bar.get_width()
                if nombres[i] == 'RSI':
                    ax_tech.text(width + 0.05, bar.get_y() + bar.get_height()/2,
                               f'{rsi:.1f}', va='center')
                elif nombres[i] == 'MACD':
                    ax_tech.text(width + 0.05, bar.get_y() + bar.get_height()/2,
                               f'{macd:.4f}', va='center')
                    ...
                    ax_tech.text(width + 0.05, bar.get_y() + bar.get_height()/2,
                               f'{stoch_k:.1f}', va='center')
                elif nombres[i] == 'ADX':
                    ax_tech.text(width + 0.05, bar.get_y() + bar.get_height()/2,
                               f'{adx:.1f}', va='center')
                elif nombres[i] == 'Volatilidad':
                    ax_tech.text(width + 0.05, bar.get_y() + bar.get_height()/2,
                               f'{volatility*100:.2f}%', va='center')
        ...
        bars1 = ax_probs.bar(names, probs, width=0.4, color=colors, alpha=0.5, label='Probabilidad')
        
        # Dibujar barras de fuerza
        bars2 = ax_probs.bar([n + 0.4 for n in range(len(names))], strengths, width=0.4, 
                           color=colors, alpha=0.8, label='Fuerza')
    ...
    def crear_panel_decision(self, fig, resultado_prediccion):
        ...
        ax.text(0.5, 0.7, f"{emoji} {decision} {emoji}", fontsize=32, weight='bold', 
    ...
    def crear_grafico_avanzado(self, fig, df, df_indicadores, prediccion_futura=None, 
                             mostrar_ma=True, mostrar_bb=True, mostrar_signals=True):
        """
        ...
        if mostrar_ma and df_indicadores is not None:
            if 'ma7' in df_indicadores.columns:
                ax_precios.plot(df_indicadores['datetime'], df_indicadores['ma7'], 
                               color=COLORES['verde'], linewidth=1, alpha=0.8, label='MA7')
            if 'ma21' in df_indicadores.columns:
                ax_precios.plot(df_indicadores['datetime'], df_indicadores['ma21'], 
                               color=COLORES['rojo'], linewidth=1, alpha=0.8, label='MA21')
            if 'ma50' in df_indicadores.columns:
                ax_precios.plot(df_indicadores['datetime'], df_indicadores['ma50'], 
                               color=COLORES['azul'], linewidth=1, alpha=0.6, label='MA50')
        
        # Dibujar bandas de Bollinger si está habilitado
        if mostrar_bb and df_indicadores is not None:
            if all(col in df_indicadores.columns for col in ['bb_upper', 'bb_lower']):
                ax_precios.plot(df_indicadores['datetime'], df_indicadores['bb_upper'], 
                               color=COLORES['gris'], linestyle='--', linewidth=1, alpha=0.6)
                ax_precios.plot(df_indicadores['datetime'], df_indicadores['bb_lower'], 
                               color=COLORES['gris'], linestyle='--', linewidth=1, alpha=0.6)
        ...
        if prediccion_futura is not None:
            ...
            if fechas_futuras and precios_futuros:
                ...
                ax_precios.plot(fechas_futuras, precios_futuros, color=color_pred, 
                               linestyle='--', linewidth=2, label='Predicción')
        ...
        for i in range(len(df)):
            if df.iloc[i]['close'] >= df.iloc[i]['open']:
                ...
                color = COLORES['rojo_claro']
            ax_vol.bar(df.iloc[i]['datetime'], df.iloc[i]['volume'], 
                     color=color, alpha=0.7, width=0.7)
        
        # Dibujar indicadores en el panel inferior
        if df_indicadores is not None:
            if 'rsi' in df_indicadores.columns:
                ax_ind.plot(df_indicadores['datetime'], df_indicadores['rsi'], 
        ...
        symbol = df_indicadores['symbol'].iloc[0] if df_indicadores is not None and 'symbol' in df_indicadores.columns else ""
    ...
    def crear_grafico_multiplot(self, fig, df, indicadores_mostrar=None):
        ...
        indicadores_disponibles = [ind for ind in indicadores_mostrar if ind in df.columns]
        ...
        for i, indicador in enumerate(indicadores_disponibles, 1):
            ...
            if indicador == 'rsi':
                ...
                ax_ind.plot(df['datetime'], df[indicador], color=COLORES['azul'], label='MACD')
                if 'macd_signal' in df.columns:
                    ax_ind.plot(df['datetime'], df['macd_signal'], color=COLORES['naranja'], 
                               label='Signal')
                if 'macd_hist' in df.columns:
                    ...
                    for j in range(len(df)):
                        if df.iloc[j]['macd_hist'] > 0:
                            ...
                            color = COLORES['rojo_claro']
                        ax_ind.bar(df.iloc[j]['datetime'], df.iloc[j]['macd_hist'], 
                                 color=color, alpha=0.7, width=0.7)
                ...
                ax_ind.plot(df['datetime'], df[indicador], 
                           color=COLORES['azul'] if indicador == 'stoch_k' else COLORES['verde'])
                if 'stoch_d' in df.columns and indicador == 'stoch_k':
                    ax_ind.plot(df['datetime'], df['stoch_d'], color=COLORES['verde'], 
                               label='Stoch D')
                ...
                ax_ind.axhline(y=media_bb_width, color=self.color_texto, 
                              linestyle='--', alpha=0.5)
                
            elif indicador == 'adx':
                ax_ind.plot(df['datetime'], df[indicador], color=COLORES['morado'], label='ADX')
                if 'plus_di' in df.columns and 'minus_di' in df.columns:
                    ax_ind.plot(df['datetime'], df['plus_di'], color=COLORES['verde'], 
                               label='+DI')
                    ax_ind.plot(df['datetime'], df['minus_di'], color=COLORES['rojo'], 
                               label='-DI')
