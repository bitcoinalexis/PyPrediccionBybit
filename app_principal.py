...
from visualizaciones import Visualizador, COLORES
...
class AplicacionPredictor:
    ...
    def __init__(self, root):
        ...
        self.fig_precios = plt.figure(figsize=(10, 6))
        self.fig_mapa_calor = plt.figure(figsize=(8, 6))
        self.fig_indicadores = plt.figure(figsize=(6, 4))
        self.fig_decision = plt.figure(figsize=(4, 3))
    ...
    def configurar_tema(self):
        ...
        style.configure('TNotebook.Tab', background=self.widget_bg, foreground=self.fg_color)
    ...
    def setup_trading_tab(self, parent):
        ...
        symbol_combo = ttk.Combobox(control_frame, textvariable=self.symbol_var, width=10)
        symbol_combo['values'] = ("BTCUSDT", "ETHUSDT", "SOLUSDT", "ADAUSDT", "BNBUSDT", "XRPUSDT")
        ...
        interval_combo = ttk.Combobox(control_frame, textvariable=self.intervalo_var, width=5)
        interval_combo['values'] = ("1", "5", "15", "30", "60", "240", "D")
        interval_combo.pack(side=tk.LEFT, padx=5)
        
        # Botones de acción
        ttk.Button(control_frame, text="Actualizar", command=self.actualizar_datos).pack(side=tk.LEFT, padx=5)
        
        # Monitoreo
        self.monitoreo_btn = ttk.Button(control_frame, text="Iniciar Monitoreo", command=self.toggle_monitoreo)
    ...
    def setup_config_tab(self, parent):
        ...
        # API Secret
        ttk.Label(api_frame, text="API Secret:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        secret_entry = ttk.Entry(api_frame, textvariable=self.api_secret_var, width=50, show="*")
        secret_entry.grid(row=1, column=1, padx=5, pady=5)
        
        # Frame para configuración de modelo
        model_frame = ttk.LabelFrame(parent, text="Configuración del Modelo", padding="10")
        model_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Umbral de probabilidad
        ttk.Label(model_frame, text="Umbral de Probabilidad:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        threshold_scale = ttk.Scale(model_frame, from_=0.5, to=0.95, orient=tk.HORIZONTAL, 
                                  variable=self.umbral_prob_var, length=200)
        ...
        # Modo oscuro toggle
        dark_check = ttk.Checkbutton(appearance_frame, text="Modo Oscuro", variable=self.modo_oscuro, 
        ...
        ttk.Button(button_frame, text="Probar Conexión", 
                 command=self.probar_conexion).pack(side=tk.LEFT, padx=5)
        
        # Botón para entrenar modelo
        ttk.Button(button_frame, text="Entrenar Modelos", 
                 command=self.entrenar_modelos).pack(side=tk.LEFT, padx=5)
        
        # Botón para actualizar lista de pares
        ttk.Button(button_frame, text="Actualizar Pares", 
                 command=self.actualizar_lista_pares).pack(side=tk.LEFT, padx=5)
    ...
    def setup_chart_tab(self, parent):
        ...
        ttk.Label(control_frame, text="Opciones de Visualización:").pack(side=tk.LEFT, padx=5)
        
        # Checkbuttons para diferentes indicadores
        self.show_ma = tk.BooleanVar(value=True)
        ttk.Checkbutton(control_frame, text="Medias Móviles", 
                      variable=self.show_ma).pack(side=tk.LEFT, padx=5)
        
        self.show_bb = tk.BooleanVar(value=True)
        ttk.Checkbutton(control_frame, text="Bandas Bollinger", 
                      variable=self.show_bb).pack(side=tk.LEFT, padx=5)
        
        self.show_signals = tk.BooleanVar(value=True)
        ttk.Checkbutton(control_frame, text="Señales", 
                      variable=self.show_signals).pack(side=tk.LEFT, padx=5)
        
        # Botón para actualizar
        ttk.Button(control_frame, text="Actualizar Gráfico", 
                 command=self.actualizar_grafico_avanzado).pack(side=tk.RIGHT, padx=5)
    ...
    def setup_heatmap_tab(self, parent):
        ...
        ttk.Button(control_left, text="Actualizar", 
                 command=self.actualizar_mapa_correlacion).pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Label(control_right, text="Mapa de Calor Dinámico:").pack(side=tk.LEFT, padx=5)
        
        ttk.Label(control_right, text="Períodos:").pack(side=tk.LEFT, padx=5)
        self.periodos_var = tk.StringVar(value="20")
        periodos_combo = ttk.Combobox(control_right, textvariable=self.periodos_var, width=5)
        periodos_combo['values'] = ("10", "20", "50", "100")
        periodos_combo.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(control_right, text="Actualizar", 
                 command=self.actualizar_mapa_dinamico).pack(side=tk.RIGHT, padx=5)
    ...
    def setup_log_tab(self, parent):
        ...
        ttk.Label(control_frame, text="Registro de Actividad:").pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text="Limpiar Log", 
                 command=self.limpiar_log).pack(side=tk.RIGHT, padx=5)
    ...
    def guardar_configuracion(self):
        ...
        self.status_var.set(f"Configuración guardada. Par: {self.symbol_var.get()}, Intervalo: {self.intervalo_var.get()}")
    ...
    def probar_conexion(self):
        ...
        try:
            ...
            result = self.api.obtener_datos_mercado(symbol, intervalo=self.intervalo_var.get(), limit=10)
            ...
            if result:
                ...
                messagebox.showerror("Error", "No se pudo establecer conexión con la API")
    ...
    def entrenar_modelos(self):
        ...
        try:
            ...
            if success:
                ...
                mensaje = (f"Modelos entrenados correctamente\n"
                         f"Precisión Long: {long_acc:.2f}\n"
                         f"Precisión Short: {short_acc:.2f}")
                
                messagebox.showinfo("Entrenamiento", mensaje)
                self.status_var.set(f"Modelos entrenados - Long: {long_acc:.2f}, Short: {short_acc:.2f}")
                self.log(f"Modelos entrenados. Long acc: {long_acc:.2f}, Short acc: {short_acc:.2f}", "success")
                
                # Logging de features más importantes
                self.log("Top 5 features para Long:", "info")
                long_importance = sorted(result['long_feature_importance'].items(), 
                ...
                short_importance = sorted(result['short_feature_importance'].items(), 
                                       key=lambda x: x[1], reverse=True)[:5]
    ...
    def actualizar_datos(self):
        ...
        try:
            ...
            # Klines
            kline_data = self.api.obtener_datos_mercado(symbol, intervalo=intervalo, limit=200)
            if not kline_data:
                messagebox.showerror("Error", "No se pudieron obtener datos del mercado")
            ...
            self.df_indicadores = self.analizador.calcular_indicadores(self.df_actual)
            
            # Order book
            self.order_book_actual = self.api.obtener_book_orders(symbol)
            order_book_metrics = self.analizador.procesar_order_book(self.order_book_actual)
            ...
            if hasattr(self.analizador, 'model_long') and self.analizador.model_long is not None:
                ...
                if success:
                    ...
                    self.log(f"Predicción para {symbol}: {decision}", 
                          "success" if decision != "NEUTRAL" else "info")
                    self.log(f"  Long prob: {long_prob:.2f}, Short prob: {short_prob:.2f}", "info")
                    ...
                    self.prediccion_actual = None
                    self.status_var.set(f"Datos actualizados. Sin predicción")
                    self.log(f"No se pudo generar predicción: {self.prediccion_actual}", "warning")
            else:
                self.prediccion_actual = None
                self.status_var.set(f"Datos actualizados. Entrene los modelos primero")
                self.log("Datos obtenidos. Los modelos necesitan ser entrenados", "info")
            
            # Actualizar visualizaciones
            self.actualizar_visualizaciones()
            
            # Log de información relevante
            self.log(f"Actualizados {len(self.df_actual)} registros para {symbol} ({intervalo}min)", "info")
            ultimo_precio = self.df_actual["close"].iloc[-1] if self.df_actual is not None else None
            if ultimo_precio:
    ...
    def actualizar_visualizaciones(self):
        ...
        try:
            ...
            prediccion_futura = self.analizador.generar_datos_prediccion_futura(self.df_actual)
            
            # Actualizar gráfico de precios
            self.fig_precios = self.visualizador.crear_grafico_precios(
                self.fig_precios, self.df_actual, prediccion_futura, 
                self.order_book_actual, f"{self.symbol_var.get()} ({self.intervalo_var.get()}min)"
            )
    ...
    def actualizar_lista_pares(self):
        ...
        try:
            ...
            if not tickers:
                messagebox.showwarning("Advertencia", "No se pudieron obtener los pares disponibles")
                return
                
            # Filtrar los pares con USDT
            usdt_pairs = [ticker.get('symbol') for ticker in tickers 
                         if ticker.get('symbol', '').endswith('USDT')]
            
            # Seleccionar los más populares o de mayor volumen
            if len(usdt_pairs) > 20:
                # Aquí podríamos ordenar por volumen
                popular_pairs = usdt_pairs[:20]
            else:
                popular_pairs = usdt_pairs
                
            # Actualizar comboboxes
            for combo in self.root.winfo_children():
                if isinstance(combo, ttk.Combobox) and combo.cget('values'):
                    if 'BTCUSDT' in combo.cget('values'):
                        combo['values'] = popular_pairs
                        
            # Log de la actualización
            self.log(f"Lista de pares actualizada, {len(popular_pairs)} pares disponibles", "info")
        except Exception as e:
    ...
    def toggle_monitoreo(self):
        ...
        if self.monitoreo_activo:
            ...
            if not hasattr(self.analizador, 'model_long') or self.analizador.model_long is None:
                messagebox.showwarning("Advertencia", "Debe entrenar los modelos primero")
                self.log("Intento de iniciar monitoreo sin modelos entrenados", "warning")
                return
    ...
    def monitoreo_loop(self):
        ...
        while self.monitoreo_activo:
            try:
                ...
                intervalo_min = int(self.intervalo_var.get()) if self.intervalo_var.get().isdigit() else 15
    ...
    def actualizar_grafico_avanzado(self):
        ...
        try:
            ...
            for i in range(len(self.df_actual)):
                ...
                ax_precios.plot(
                    [self.df_actual.iloc[i]['datetime'], self.df_actual.iloc[i]['datetime']], 
            ...
            if self.show_ma.get():
                if 'ma7' in self.df_indicadores.columns:
                    ax_precios.plot(
                        self.df_indicadores['datetime'], self.df_indicadores['ma7'], 
                        color=COLORES['verde'], linewidth=1, alpha=0.8, label='MA7'
                    )
                if 'ma21' in self.df_indicadores.columns:
                    ax_precios.plot(
                        self.df_indicadores['datetime'], self.df_indicadores['ma21'], 
                        color=COLORES['rojo'], linewidth=1, alpha=0.8, label='MA21'
                    )
                if 'ma50' in self.df_indicadores.columns:
                    ax_precios.plot(
                        self.df_indicadores['datetime'], self.df_indicadores['ma50'], 
                        color=COLORES['azul'], linewidth=1, alpha=0.5, label='MA50'
                    )
            
            # Añadir bandas de Bollinger si la opción está activada
            if self.show_bb.get():
                if all(col in self.df_indicadores.columns for col in ['bb_upper', 'bb_lower']):
                    ax_precios.plot(
                        self.df_indicadores['datetime'], self.df_indicadores['bb_upper'], 
                        color=COLORES['gris'], linestyle='--', linewidth=1, alpha=0.6
                    )
                    ax_precios.plot(
                        self.df_indicadores['datetime'], self.df_indicadores['bb_lower'], 
                        color=COLORES['gris'], linestyle='--', linewidth=1, alpha=0.6
                    )
            ...
            if self.show_signals.get():
                if 'target_long' in self.df_indicadores.columns:
                    ...
                    indices_compra = self.df_indicadores.index[self.df_indicadores['target_long'] == 1]
                    if len(indices_compra) > 0:
                        fechas_compra = self.df_indicadores.loc[indices_compra, 'datetime']
                        precios_compra = self.df_indicadores.loc[indices_compra, 'low'] * 0.998
                        ax_precios.scatter(
                            fechas_compra, precios_compra, marker='^', s=60, 
                            color=COLORES['verde'], alpha=0.7, label='Long Signal'
                        )
                
                if 'target_short' in self.df_indicadores.columns:
                    # Señales de venta (short)
                    indices_venta = self.df_indicadores.index[self.df_indicadores['target_short'] == 1]
                    if len(indices_venta) > 0:
                        fechas_venta = self.df_indicadores.loc[indices_venta, 'datetime']
                        precios_venta = self.df_indicadores.loc[indices_venta, 'high'] * 1.002
                        ax_precios.scatter(
                            fechas_venta, precios_venta, marker='v', s=60, 
                            color=COLORES['rojo'], alpha=0.7, label='Short Signal'
                        )
            ...
            if prediccion_futura:
                ...
                if fechas_futuras and precios_futuros:
                    ...
                    ax_precios.plot(
                        fechas_futuras, precios_futuros, color=color_pred, 
            ...
            for i in range(len(self.df_actual)):
                ...
                ax_vol.bar(
                    self.df_actual.iloc[i]['datetime'], self.df_actual.iloc[i]['volume'], 
            ...
            if 'rsi' in self.df_indicadores.columns:
                ax_ind.plot(
                    self.df_indicadores['datetime'], self.df_indicadores['rsi'], 
                    color=COLORES['morado'], label='RSI'
                )
            ...
            messagebox.showerror("Error", f"Error al actualizar gráfico avanzado: {str(e)}")
    ...
    def actualizar_mapa_dinamico(self):
        ...
        try:
            ...
            if heatmap_df is not None:
                ...
                self.log(f"Mapa de calor dinámico actualizado ({periodos} periodos)", "info")
            else:
